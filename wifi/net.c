...
int espd_net_send_ready(void) { return s_net_send_ready; }
...
void tcpreceivertask(void *z) {
  ...
  while ((err = connect(newsocket, (struct sockaddr *)&dest_addr,
                        sizeof(dest_addr)) < 0)) {
  ...
  char *rx_buffer = malloc(4000);
  if (!rx_buffer) {
    ESP_LOGE(TAG, "tcpreceivertask: failed to allocate rx_buffer");
    close(newsocket);
    return;
  }

  while (1) {
    int len = recv(newsocket, rx_buffer, 4000 - 1, 0);
    ...
    if (len < 0) {
      ...
      pd_fromhost(rx_buffer, (int)strlen(rx_buffer));
    }
  }
}
...
void udpreceivertask(void *z) {
  ...
  if (setsockopt(udpsocket, SOL_SOCKET, SO_RCVTIMEO, &timeout,
                 sizeof(timeout)) < 0) {
    ESP_LOGW(TAG, "udp recv timeout setsockopt failed");
  }

  char *rx_buffer = malloc(1000);
  if (!rx_buffer) {
    ESP_LOGE(TAG, "udpreceivertask: failed to allocate rx_buffer");
    close(udpsocket);
    return;
  }

  while (1) {
    int len = recv(udpsocket, rx_buffer, 1000 - 1, 0);
...
void net_init(void) {
  ...
  /*
   * Pin receiver tasks to Core 0 to prevent CPU contention with CPU 1 audio.
   * Stacks are allocated from internal RAM (DMA-capable).
   * Keep stack sizes minimal (buffers are dynamically allocated on heap).
   */
  xTaskCreatePinnedToCore(tcpreceivertask, "tcprcv", 8192, NULL, PRIORITY_WIFI,
                          NULL, 0);
  xTaskCreatePinnedToCore(udpreceivertask, "udprcv", 4096, NULL, PRIORITY_WIFI,
                          NULL, 0);
  s_net_send_ready = 1;
}
...
void net_sendudp(void *msg, int len, int port) {
  ...
  err = sendto(udp_out_sock, msg, len, 0, (struct sockaddr *)&udp_out_addr,
               sizeof(udp_out_addr));
  if (err < 0) {
...
void net_sendtcp(void *msg, int len) {
  ...
  if (err < 0) {
    ...
    if (errorcount < 10 || (errorcount < 100 && !(errorcount % 10)) ||
        !(errorcount % 100)) {
      ESP_LOGE(TAG, "tcp net send error %d, count %d", errno, errorcount);
    }
  }
}
...
void net_hello(void) {
  ...
  uint8_t mac[6];
  {
    ...
    sprintf(buf, "hello %02x:%02x:%02x:%02x:%02x:%02x %s;\n", mac[0], mac[1],
            mac[2], mac[3], mac[4], mac[5], wifi_ipaddr);
