...
static void *espd_i2c_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    if (bus < 0 || bus >= ESPD_I2C_MAX_BUSES) {
        pd_error(x, "espd_i2c: bus must be 0..%d", ESPD_I2C_MAX_BUSES - 1);
        bus = 0;
    }
    if (addr < 0 || addr > 127) {
        pd_error(x, "espd_i2c: 7-bit address must be 0..127");
        addr = 0x68;
    }
    ...
    if (!espd_i2c_bus_ready(bus))
        pd_error(x, "espd_i2c: bus %d not configured in config.txt", bus);
    return x;
}

void espd_pd_i2c_setup(void)
{
    espd_i2c_class = class_new(gensym("espd_i2c"),
