...
esp_err_t espd_audio_init(espd_audio_t **out)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to create DAC continuous channel: %s", esp_err_to_name(ret));
        free(a);
        return ret;
    }
    ...
    *out = a;
    const char *chan_str = (a->chan_mask == DAC_CHANNEL_MASK_CH1) ? "CH1 (GPIO26)" :
                           (a->chan_mask == DAC_CHANNEL_MASK_CH0) ? "CH0 (GPIO25)" :
                           "CH0+CH1 (GPIO25+26)";
    ESP_LOGI(TAG, "ready: %d Hz, %d channel(s) on DAC %s", a->sample_rate, a->channels, chan_str);
    return ESP_OK;
}
    ...
    size_t samples)
{
    ...
    bool stereo_dac = (audio->chan_mask == (DAC_CHANNEL_MASK_CH0 | DAC_CHANNEL_MASK_CH1));
    ...
    ret = dac_continuous_write(audio->dac_handle, dac_buf, dac_samples, &bytes_written, portMAX_DELAY);
